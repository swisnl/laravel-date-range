...
class HasDateRangeBothDefaultNameNullableTest extends TestCase
{
    ...
    public function it_can_query_active(): void
    {
        ...
        $active = BothDefaultNameNullable::whereActive()->get();

        $this->assertCount(12, $active);
        $this->assertTrue($active->every(fn (BothDefaultNameNullable $model) => $model->isActive()));
    }
    ...
    public function it_can_query_past(): void
    {
        ...
        $past = BothDefaultNameNullable::wherePast()->get();

        $this->assertCount(12, $past);
        $this->assertTrue($past->every(fn (BothDefaultNameNullable $model) => $model->isPast()));
    }
    ...
    public function it_can_query_future(): void
    {
        ...
        $future = BothDefaultNameNullable::whereFuture()->get();

        $this->assertCount(12, $future);
        $this->assertTrue($future->every(fn (BothDefaultNameNullable $model) => $model->isFuture()));
    }
    ...
    public function it_can_query_past_or_future(): void
    {
        ...
        $models = BothDefaultNameNullable::wherePast()->orWhereFuture()->get();

        $this->assertCount(51, $models);
        $this->assertTrue($models->every(fn (BothDefaultNameNullable $model) => $model->isPast() || $model->isFuture()));

        /** @var \Illuminate\Support\Collection<array-key, \Workbench\App\Models\BothDefaultNameNullable> $models */
        $models = BothDefaultNameNullable::whereNotActive()->get();
    ...
    public function it_can_sort_on_active(): void
    {
        ...
        /** @var \Illuminate\Support\Collection<array-key, \Workbench\App\Models\BothDefaultNameNullable> $models */
        $models = BothDefaultNameNullable::query()->orderByActive()->get();

        $this->assertCount(15, $models);

        $this->assertTrue($models->slice(0, 5)->every(fn (BothDefaultNameNullable $model) => $model->isActive()));
        $this->assertTrue($models->slice(5)->every(fn (BothDefaultNameNullable $model) => $model->isPast() || $model->isFuture()));
    }
