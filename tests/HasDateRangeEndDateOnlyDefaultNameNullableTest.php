...
class HasDateRangeEndDateOnlyDefaultNameNullableTest extends TestCase
{
    ...
    public function it_can_query_active(): void
    {
        ...
        $active = EndDateOnlyDefaultNameNullable::whereDateRangeActive()->get();

        $this->assertCount(4, $active);
        $this->assertTrue($active->every(fn (EndDateOnlyDefaultNameNullable $model) => $model->isDateRangeActive()));
    }
    ...
    public function it_can_query_past(): void
    {
        ...
        $past = EndDateOnlyDefaultNameNullable::whereDateRangePast()->get();

        $this->assertCount(4, $past);
        $this->assertTrue($past->every(fn (EndDateOnlyDefaultNameNullable $model) => $model->isDateRangePast()));
    }
    ...
    public function it_can_query_future(): void
    {
        ...
        $future = EndDateOnlyDefaultNameNullable::whereDateRangeFuture()->get();
    ...
    public function it_can_sort_on_active(): void
    {
        ...
        /** @var \Illuminate\Support\Collection<array-key, \Workbench\App\Models\EndDateOnlyDefaultNameNullable> $models */
        $models = EndDateOnlyDefaultNameNullable::orderByDateRangeActive()->get();

        $this->assertCount(8, $models);

        $this->assertTrue($models->slice(0, 5)->every(fn (EndDateOnlyDefaultNameNullable $model) => $model->isDateRangeActive()));
        $this->assertTrue($models->slice(5)->every(fn (EndDateOnlyDefaultNameNullable $model) => $model->isDateRangePast() || $model->isDateRangeFuture()));
    }
