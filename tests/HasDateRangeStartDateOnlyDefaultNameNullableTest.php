...
class HasDateRangeStartDateOnlyDefaultNameNullableTest extends TestCase
{
    ...
    public function it_can_query_active(): void
    {
        ...
        $active = StartDateOnlyDefaultNameNullable::whereActive()->get();

        $this->assertCount(4, $active);
        $this->assertTrue($active->every(fn (StartDateOnlyDefaultNameNullable $model) => $model->isActive()));
    }
    ...
    public function it_can_query_past(): void
    {
        ...
        $past = StartDateOnlyDefaultNameNullable::wherePast()->get();
    ...
    public function it_can_query_future(): void
    {
        ...
        $future = StartDateOnlyDefaultNameNullable::whereFuture()->get();

        $this->assertCount(4, $future);
        $this->assertTrue($future->every(fn (StartDateOnlyDefaultNameNullable $model) => $model->isFuture()));
    }
    ...
    public function it_can_sort_on_active(): void
    {
        ...
        /** @var \Illuminate\Support\Collection<array-key, \Workbench\App\Models\StartDateOnlyDefaultNameNullable> $models */
        $models = StartDateOnlyDefaultNameNullable::query()->orderByActive()->get();

        $this->assertCount(12, $models);

        $this->assertTrue($models->slice(0, 5)->every(fn (StartDateOnlyDefaultNameNullable $model) => $model->isActive()));
        $this->assertTrue($models->slice(5)->every(fn (StartDateOnlyDefaultNameNullable $model) => $model->isPast() || $model->isFuture()));
    }
