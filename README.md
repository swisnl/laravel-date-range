...
// the date range).
\App\Models\MyModel::query()->whereActive()->get(); // default to today
\App\Models\MyModel::query()->whereActive('2023-01-15')->get();

// Scope to filter models where the date range is in the past (ends before
// the given date).
\App\Models\MyModel::query()->wherePast()->get(); // default to today
\App\Models\MyModel::query()->wherePast('2023-01-15')->get();

// Scope to filter models where the date range is in the future (starts after
// the given date).
\App\Models\MyModel::query()->whereFuture()->get(); // default to today
\App\Models\MyModel::query()->whereFuture('2023-01-15')->get();

// Order models to first show active models and then inactive models.
\App\Models\MyModel::query()->orderByActive()->get(); // default to today
\App\Models\MyModel::query()->orderByActive('2023-01-15')->get();
\App\Models\MyModel::query()->orderByActive('2023-01-15', 'asc')->get(); // inactive first
```
...
```php
// Equivalent queries
\App\Models\MyModel::query()->whereNotActive()->get();
\App\Models\MyModel::query()->wherePast()->orWhereFuture()->get();
```
